...
import {XVIZFilesSession} from './xviz-files-session';

// Setup the source and return an XVIZSession or null
export class XVIZFilesHandler {
  constructor(options) {
  ...
  async newSession(socket, req) {
    ...
    for (let i = 0; i < dirs.length; i++) {
      ...
      if (source.existsSync('2-frame.pbe')) {
        return new XVIZFilesSession(socket, req, source, {
          ...this.options,
