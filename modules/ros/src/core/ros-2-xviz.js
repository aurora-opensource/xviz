...
export class ROS2XVIZ {
  ...
  _makeConvertersForTopic(instances, topicMessage, mappedState, aux) {
    const {topic, type, message} = topicMessage;
    ...
    if (mapping) {
      ...
      for (let [index, entry] of mapping.entries()) {
        ...
        if (entry.topic === topic) {
          ...
          if (entry.name) {
            ...
            if (!Converter) {
              this.log(`ROS2XVIZ cannot find the converter with name ${entry.name} for topic ${topic}`);
            }
          } else {
  ...
  initializeConverters(topicMessageTypes, aux) {
    ...
    for (let entry of topicMessageTypes) {
