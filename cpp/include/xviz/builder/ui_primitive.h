...
class XVIZTreeTableRowBuilder {
 public:
  XVIZTreeTableRowBuilder(int id, const std::vector<std::string>& values,
                          std::optional<int> parent = std::nullopt);
  XVIZTreeTableRowBuilder(int id, std::vector<std::string>&& values,
                          std::optional<int> parent = std::nullopt);

  XVIZTreeTableRowBuilder& Children(int id,
...
class XVIZUIPrimitiveBuilder : public XVIZBaseBuilder {
 public:
  ...
  // XVIZUIPrimitiveBuilder& TreeTable(const std::vector<TreeTableColumn>&
  // tree_table_columns); XVIZUIPrimitiveBuilder&
  // TreeTable(std::vector<TreeTableColumn>&& tree_table_columns);
  XVIZUIPrimitiveBuilder& Column(
      const std::string& display_text,
      xviz::TreeTableColumn::ColumnType type_id,
