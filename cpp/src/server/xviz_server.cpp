...
std::unordered_map<std::string, std::string> xviz::ParseURIParameters(
  ...
  while ((and_sign = uri.find('&', last_and_sign_pos)) != std::string::npos) {
    auto key_value_pair = ParseOnePair(
        uri.substr(last_and_sign_pos, and_sign - last_and_sign_pos));
...
XVIZServer::XVIZServer(
    const std::vector<std::shared_ptr<XVIZBaseHandler>>& handlers,
    uint16_t port, int max_payload, bool per_message_defalte)
    : handlers_(handlers),
      port_(port),
      max_payload_{max_payload},
      per_message_deflate_(per_message_defalte) {
  InitInternalServer();
}
XVIZServer::XVIZServer(std::vector<std::shared_ptr<XVIZBaseHandler>>&& handlers,
                       uint16_t port, int max_payload, bool per_message_defalte)
...
void XVIZServer::InitInternalServer() {
  internal_server_ptr_ =
      std::make_shared<websocketpp::server<websocketpp::config::asio>>();
  ...
  internal_server_ptr_->set_open_handler(
      std::bind(&XVIZServer::HandleSession, this, std::placeholders::_1));
...
void XVIZServer::HandleSession(websocketpp::connection_hdl hdl) {
  ...
  for (const auto& handler_ptr : handlers_) {
    ...
    if (session_ptr) {
      std::thread t(std::bind(&XVIZServer::SessionThread, this, session_ptr));
      t.detach();
      break;
    }
  }
...
void XVIZServer::Serve() { internal_server_ptr_->run(); }
