...
xviz::test::GetBuilderTestMetadataBuilderForPrimitive() {
  ...
  std::vector<std::string> suffix = {"/copy", "/move", "/pointer"};
  uint32_t cnt = 0u;
  metadata_builder.Stream("/primitive")
      .Category(xviz::StreamMetadata::PRIMITIVE)
      .Type(xviz::StreamMetadata::POLYGON);
  for (auto primitive_type = xviz::StreamMetadata::PrimitiveType::
           StreamMetadata_PrimitiveType_CIRCLE;
       primitive_type <=
       xviz::StreamMetadata::PrimitiveType::StreamMetadata_PrimitiveType_TEXT;
       primitive_type =
           (xviz::StreamMetadata::PrimitiveType)((int)primitive_type + 1)) {
    for (auto cnt = 0u; cnt < 3u; cnt++) {
      std::string name =
          "/primitive/" +
          xviz::StreamMetadata::PrimitiveType_Name(primitive_type);
      name += suffix[cnt];
...
xviz::XVIZMetadataBuilder
xviz::test::GetBuilderTestMetadataBuilderForTimeSeries() {
  xviz::XVIZMetadataBuilder metadata_builder;
  metadata_builder.Stream("/vehicle_pose").Category(xviz::StreamMetadata::POSE);
  for (auto ts_type =
           xviz::StreamMetadata::ScalarType::StreamMetadata_ScalarType_FLOAT;
       ts_type <=
       xviz::StreamMetadata::ScalarType::StreamMetadata_ScalarType_BOOL;
...
xviz::XVIZMetadataBuilder
xviz::test::GetBuilderTestMetadataBuilderForUIPrimitive() {
  xviz::XVIZMetadataBuilder metadata_builder;
  metadata_builder.Stream("/vehicle_pose").Category(xviz::StreamMetadata::POSE);
...
std::string xviz::test::ConvertBinaryToReadableChar(const std::string& input) {
  ...
  for (unsigned char c : input) {
    if (std::isprint(c)) {
      ...
      tmp_oss << "\\x" << std::setw(2) << std::setfill('0') << std::hex
              << (int)c;
      oss << tmp_oss.str();
    }
  }
